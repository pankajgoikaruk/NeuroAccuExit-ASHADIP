...
#
# Example: continue new silence clips after the existing seed collection.
#
# Dry run:
...
#
# Important:
#   --root must contain immediate class folders. For example:
...
def safe_label_name(name: str, preserve_case: bool = False) -> str:
    """Convert a class-folder name into a safe filename prefix."""
    name = name.strip()
...
def collect_audio_files(class_dir: Path) -> list[Path]:
    ...
    This intentionally does not recurse, avoiding generated segments or nested
    output folders.
    """
    return sorted(
        [
...
def parse_existing_index_from_name(
    ...
    """
    Return the numeric index from names such as:
      silence__0001.wav
      class_0001.flac
    """
...
def parse_existing_index(
    ...
    """Path wrapper retained for backward compatibility."""
...
def make_index(index: int, digits: int) -> str:
    return str(index).zfill(digits) if digits > 0 else str(index)


def resolve_manifest_path(root: Path, manifest_arg: str) -> Path:
    """
    Resolve output manifest while preserving the original simple-name behavior.
    """
...
def iter_reference_audio_names(reference_root: Path):
...
def iter_reference_manifest_names(reference_manifest: Path):
    """
    Yield filenames from known filename/path columns in a CSV manifest.

    The manifest must contain at least one supported reference column.
    """
    with reference_manifest.open("r", newline="", encoding="utf-8-sig") as handle:
        reader = csv.DictReader(handle)
        fieldnames = reader.fieldnames or []
        matched_columns = [
            column for column in REFERENCE_FILENAME_COLUMNS
...
def collect_reference_names(
    ...
    """Collect lowercase reference filenames from a dataset and/or manifest."""
    names: set[str] = set()
    if reference_root is not None:
        names.update(name.lower() for name in iter_reference_audio_names(reference_root))
    if reference_manifest is not None:
        names.update(
            name.lower()
            for name in iter_reference_manifest_names(reference_manifest)
        )
    return names
...
def reference_indices_for_label(
    ...
    """Extract all matching numeric indices for one class prefix."""
    indices: list[int] = []
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Root containing immediate class folders. Example: "
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Minimum starting index for each class. When a reference source "
            "contains larger indices, numbering continues after its maximum."
        ),
    )
    parser.add_argument(
        "--digits",
        type=int,
        default=4,
        help="Zero padding. Example: 4 gives silence__0001.wav.",
    )
    parser.add_argument(
        "--separator",
        default="_",
        help="Separator between class prefix and index. Use '__' for silence__0001.wav.",
    )
    parser.add_argument(
        "--preserve_case",
        action="store_true",
        help="Preserve class-folder case in output prefixes.",
    )
    parser.add_argument(
        "--manifest",
        default="rename_manifest.csv",
        help="CSV path for old/new rename mappings.",
    )
    parser.add_argument(
        "--skip_already_named",
        action="store_true",
        help="Skip files already following the selected class naming pattern.",
    )
    parser.add_argument(
        "--reference_root",
        default=None,
        help=(
            ...
            "continues after its highest matching class index, and its filenames "
            "are protected from reuse."
        ),
    )
    parser.add_argument(
        "--reference_manifest",
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Actually rename files. Without this flag, perform a dry run only.",
    )
    ...
    print(f"Reference manifest: {reference_manifest or 'not supplied'}")
    ...
    for class_dir in class_dirs:
        ...
        next_index = max(args.start - 1, reference_max_index, local_max_index) + 1
        ...
        for old_path in audio_files:
            ...
            planned.append((old_path, new_path, action, assigned_index))
            next_index += 1
        ...
        if len(target_names_lower) != len(set(target_names_lower)):
            raise RuntimeError(
                f"Duplicate target filenames detected in plan: {class_dir}"
            )
        ...
        for old_path, new_path, action, _ in planned:
            ...
            if new_path.name.lower() in reference_names:
                raise RuntimeError(
                    "Target filename already exists in the protected reference "
                    "dataset or manifest:\n"
                    f"  old: {old_path}\n"
        ...
        for old_path, new_path, action, _ in planned:
            ...
            if (
                ...
                raise RuntimeError(
                    "Target file already exists and is not part of this rename "
                    "plan:\n"
                    f"  old: {old_path}\n"
                    f"  new: {new_path}\n"
                    "No files were renamed."
        ...
        if args.apply and rename_items:
            ...
            try:
                ...
                for temp_path, new_path in reversed(temp_pairs):
                    if temp_path.exists():
                        original = next(
                            (
                                old_path
                                for old_path, planned_new in rename_items
                                if planned_new == new_path
                            ),
                            None,
                        )
    ...
    if not args.apply:
        ...
        print("Inspect the manifest, then rerun the same command with --apply.")
    else:
