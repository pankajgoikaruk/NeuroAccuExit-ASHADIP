...
def policy_eval(run_dir, segments_csv, features_root, save_plots=True):
    ...
    tau = float(th["tau"])
    temps = load_json_safepath(os.path.join(run_dir, "temperature.json"), {"temperatures": [1.0, 1.0, 1.0]})[
        "temperatures"
    ]
...
def main():
    ...
    # NEW: skip appending to experiments.csv when re-generating reports
    ap.add_argument(
        "--no_log",
        action="store_true",
    ...
    # experiments.csv (append) — guarded by --no_log
    if args.no_log:
