...
def estimate_flops_tiny_audiocnn_tapblocks(n_mels: int, frames: int, num_classes: int, tap_blocks):
    ...
    Exits:
      for each tap block in sorted(tap_blocks) (allowed 1..4): head_dim = channels at that block
      final exit: after block5 head_dim=64

    Returns dict: {"exit1":..., ..., "exitK":...} cumulative FLOPs up to that exit (including its linear head).
    """
...
def append_csv_union(path: Path, row: dict):
    ...
    new_fields = list(dict.fromkeys(old_fields + list(row.keys())))  # preserve order + add new keys

    if new_fields == old_fields:
        # same header -> append filtered
    ...
    # header changed -> rewrite whole CSV with new header
    with open(path, "w", newline="", encoding="utf-8") as f:
...
def main():
    ...
    ap.add_argument("--run_dir", required=True, help="Path to a single run directory, e.g. runs/20251113_142831")
    ...
    profiling = {
        ...
        "n_mels": int(n_mels),
        "frames": int(frames),
        "latency_ms": lat_ms,   # {"exit1":..., ..., "exitK":...}
        "flops": flops,         # {"exit1":..., ..., "exitK":...}
    ...
    # --- Append to analysis/on_device_summary.csv (header auto-expands) ---
